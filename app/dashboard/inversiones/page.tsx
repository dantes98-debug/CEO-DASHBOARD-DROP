...
import { LineChart as LineChartIcon, Plus, TrendingUp, TrendingDown, Megaphone, Target, DollarSign, BarChart2, Upload, Download, BookOpen, Trash2, Pencil } from 'lucide-react'
import {
...
type Tab = 'inversiones' | 'marketing' | 'estudios'
...
export default function InversionesPage() {
  ...
  const [tab, setTab] = useState<Tab>(() => {
    ...
    return (['inversiones','marketing','estudios'] as Tab[]).includes(t as Tab) ? (t as Tab) : 'inversiones'
  })
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Inversiones, marketing y estudios de mercado"
        icon={LineChartIcon}
        action={
          tab === 'inversiones' ? (
            <button
      ...
      <div className="flex gap-1 bg-background border border-border rounded-xl p-1 mb-8 w-fit">
        {([['inversiones', LineChartIcon, 'Inversiones'], ['marketing', Megaphone, 'Pauta / Ads'], ['estudios', BookOpen, 'Estudios']] as const).map(([key, Icon, label]) => (
          <button
            key={key}
