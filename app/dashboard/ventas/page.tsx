...
export default function VentasPage() {
  ...
  const fetchData = async () => {
    ...
    const [ventasRes, clientesRes, estudiosRes, productosRes, configRes, stockRes] = await Promise.all([
      supabase.from('ventas').select('*, clientes(nombre), estudios(nombre)').or('canal.neq.ecommerce,confirmada.eq.true').gte('fecha', dosAniosAtras).order('fecha', { ascending: false }),
                      ...
                      <td colSpan={9} className="px-4 py-4">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-3">
                          ...
                          <div className="bg-card rounded-lg p-3 border border-border">
                            ...
                            <p className={`text-sm font-semibold ${(row.ganancia || 0) >= 0 ? 'text-blue-600' : 'text-red-500'}`} data-private>
